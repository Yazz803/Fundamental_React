...
function App() {
  ...
  React.useEffect(() => {
    // Ini promise, jadi proses asynchronous. Ketika code dibawah di eksekusi, dia di taro di background
    // jadi langsung ke code execution berikutnya. getData ini returnnya promise, jadi harus pake then()
    // const getData = fetch("https:///api.spaceflightnewsapi.net/v3/blogs")
    //   .then((response) => {
    //     // response.json() juga promise/async, jadi ada 2 kemungkinan response nya bisa resolve(200 ok) atau reject (500 server error)
    ...
    // Pake async await, jadi kalo mau pake await, pastikan kita eksekusi codenya di dalam function async
    async function getData() {
      const request = await fetch(
        "https:///api.spaceflightnewsapi.net/v3/blogs"
      );
      // Jadi si code ini akan di execute setelah si promise request ini resolve atau reject. Jadi request nya harus selesai dulu
